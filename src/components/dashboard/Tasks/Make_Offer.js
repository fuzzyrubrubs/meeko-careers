...
function Make_Offer (props) {
    ...
    const [match_probation_salary, set_match_probation_salary] = useState(null);
    const [match_probation_bonus, set_match_probation_bonus] = useState(null);

    const [start, set_start] = useState("");
    const [salary, set_salary] = useState(parseInt(job_data.salary*1000 / 12));
    const [bonus_pay, set_bonus_pay] = useState(null);
    const [annual_leave, set_annual_leave] = useState(null);
    const [paid_sick, set_paid_sick] = useState(null);
    const [benefits, set_benefits] = useState([]);

    const [probation_duration, set_probation_duration] = useState(null);
    ...
    const submit_handler = () => {
        ...
        // update_application_message(application_id, job_data.templates.offer);
    };
            ...
            <section className={styles.form}>
                ...
                <h4 class="bold medium">{job_data.title} at {job_data.company_name}</h4>
                    <section className={styles.grid}>
                        ...
                        <Button_Main action={() => set_selected(1)}>View Offer</Button_Main>
    ...
    const contract = (
        <main className={styles.contract}>
            ...
            <div onClick={() => set_selected(0)}><FaChevronLeft /></div>
            <Button_Main action={submit_handler}>Send Offer</Button_Main>

        </main>
    )


    const main = [form, contract]
