...
function Office (props) {
    ...
    useEffect(() => {
        ...
        new Array(7).fill().forEach((item, index) => {
            ...
            items.push({
                time: time, 
                open: days[index],
                crowd: data.rota.filter(obj => obj.date === format || obj.day === time.format('d'))
            });
        });
    ...
    const add_handler = (time, employee_id, fixed) => {
        const seconds = time.format('X');
        const day =  fixed ? time.format('d') : null
        const id = generatePushID();
        add_rota(id, employee_id, data.id, seconds, day);
    };
            ...
            <section className={styles.rota}>
               ...
               <div className={styles.rota__week}>
                    {calendar.length === 0 ? null : calendar.map((item, index) => <div className={styles.rota__day}>
                        <p class="bold">{item.time.format('dddd')} <small>9am-5pm</small></p>
                        <p>{item.time.format('X')}</p>
                        {item.crowd.map(user => <p>{user.employee_id}</p>)}
                    </div>)}
               </div>
            </section>

            <section className={styles.employees}>
                <div className={styles.employees__user}>
                    <p>John Parviti</p>
                </div>
            </section>
